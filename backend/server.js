...
app.get("/", (req, res) => {
  res.send("🚀 CampusPulse Backend Running Successfully");
});
...
app.use(
  cors({
    origin: (origin, callback) => {
      ...
      return callback(
        new Error("CORS Policy: Origin not allowed by configurations"),
        false,
      );
    },
    credentials: true,
...
app.get("/api/health", (req, res) => {
  res
    ...
    .json({
      success: true,
      message: "CampusPulse Full-Stack API is active and healthy.",
    });
});
...
app.use("/api/*", (req, res) => {
  res
    .status(404)
    .json({ success: false, message: "Requested API resource not found" });
});
...
app.listen(PORT, () => {
  console.log(
    `⚡ CampusPulse Server running in ${process.env.NODE_ENV || "development"} mode on port ${PORT}`,
  );
});
